    ...
    do{
        ...
        switch(opc){
            ...
            case 3:
                ///Hacer Abono
                    if(contCuentas==0){
                        ...
                        if(cuenta){
                            if(cuenta->getContadorAbonos()<TAM){
            ...
            case 6:
                ...
                if(contCuentas==0){
                    cout << "No hay existencia de cuentas intenta mas tarde\n";
                }else{
                    cout << "Digite el numero de cuenta para obetener sus detalles\n";
                    cin >> idCuenta;
                    cuenta = buscarCuenta(lstCuenta, idCuenta, contCuentas);
                    if(cuenta){
                        cout << "Numero de cuenta: " << cuenta->getNumeroCuenta() << "\t";
                        cout << "Nombre: " << cuenta->getCliente()->getNombre() << "\t";
                        cout << "Apellido: " << cuenta->getCliente()->getApellido() << "\t";
                        cout << "Saldo: " << cuenta ->getSaldo() << "\n";
                        cout << "Fecha\t\tMonto Abonado\n";
                        Abono **lst = cuenta->getlstAbonos();
                        for(int i=0; i<cuenta->getContadorAbonos(); i ++){
                            lst[i]->getFechaAbono()->mostrarFecha();
                            cout << "\t" << lst[i]->getMontoAbono() << "\n";
                        }
                    }else{
                        cout << "La cuenta no se encontro, intenta de nuevo\n";
                    }
                }
                break;
            case 7:
